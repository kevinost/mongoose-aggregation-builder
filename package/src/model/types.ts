...
export type IncludeIdAndTimestampsAndDefaultProperties<
    ...
    Schema extends SchemaClass<any, any>,
    Data = Model,
> = (InferConfigfFromSchema<Schema>['timestamps'] extends true ? { createdAt: Date; updatedAt: Date } : {}) & {
    _id: Types.ObjectId;
} & Required<Pick<Model, DefaultValueProperties<InferSchemaDefFromSchema<Schema>> & keyof Model>> &
    Data;
...
type OnFieldOperation<Type> = Type extends Array<infer SubType>
    ? OnFieldOperation<SubType> | { $all: Array<Type> } | { $elemMatch: OnFieldOperation<SubType> } | { $size: number }
    : Type extends number
      ?
            | number
...
export type MatchQuery<ModelI> = {
    ...
    [key in `${KeysWithValOfType<Required<ModelI>, Array<any>> & string}.${number}`]?: ModelI[RemoveDot<key> &
        keyof ModelI] extends NonNullable<Array<infer SubType>>

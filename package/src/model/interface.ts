...
> {
    ...
    ) => Promise<ApplyProj<ModelI, Proj> | { error: { status: 404; name: `${ModelName} not found`; data?: any } }>;

    findOne: <Proj extends Projection<Required<ModelI>> = CompleteProj<ModelI>>(
        filer: MatchQuery<ModelI>,
        proj?: Proj,
        options?: FindOneOptions,
    ) => Promise<ApplyProj<ModelI, Proj> | { error: { status: 404; name: `${ModelName} not found` } }>;

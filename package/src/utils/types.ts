...
export type StricProperty<T, M> = Exclude<keyof T, keyof M> extends never ? T : 'Error: Only strict properties allowed';
...
export type ExtractFromPath<Obj extends any, Path extends FlatPath<Obj>> = Path extends `${infer A}.${infer B}`
...
export type Plurial<T extends string> = T extends `${string}${'s' | 'sh' | 'ch' | 'x' | 'z'}` ? `${T}es` : `${T}s`;

import { createDB, Schema, mongoose, isError, ObjectId } from '../src';
...
describe('test', () => {
    ...
    it('first test', async () => {
        ...
        const post = await DB.post.create({ userId: user._id, text: 'something' });
        ...
        const creators = await DB.user
            .aggregate()
            .project({ name: 1 })
            .lookup({ from: 'posts', as: 'posts', localField: '_id', foreignField: 'userId' })
